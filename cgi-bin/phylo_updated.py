...
def main():
    ...
    alt_names = {
        clade: desired_name[ALT_NAME]
        for clade, desired_name in clade_details.items()
        if len(desired_name) == 1
    }
    clade_list_details = {
        clade: col_dens_cov
        for clade, col_dens_cov in clade_details.items()
        if len(col_dens_cov) > 1
    }
    ...
    color_clades(
        tree,
        clades=clades,
        clade_details=clade_list_details,
...
def get_clade_count(
    tree: TreeNode, clades: List[str], alternate_names: Dict[str, str]
) -> Dict[str, int]:
    """
    ...
    def get_belonging(leaf: TreeNode, clades: Dict[str, str]):
        """"""
        result = [
            clades[clade] for clade in clades.keys() if leaf.name.startswith(clade)
        ]
        if len(result) == 0:
            return "other"
        return result[0]
    ...
    leaf_clades = list(
        map(partial(get_belonging, clades=clade_dict), tree.get_leaves())
    )
...
def root_on(
    ...
    """
    node = get_max_ancestor(
        tree, clade, clade_details, clade_total, clades, alternate_names
    )
...
def get_node_details(
    ...
    for node in tree.traverse():
        if node.is_leaf():
            continue
        result[node] = get_clade_count(
            node, clades=clades, alternate_names=alternate_names
        )
    return result


def get_max_ancestor(
    tree: TreeNode,
    clade: Union[str, List[str]],
    clade_details: List,
    clade_total: int,
    clades: List[str] = None,
    alternate_names: Dict[str, str] = None,
    ...
    def get_qualifying_nodes(
        ...
        for node in tree.traverse():
            ...
            if node_cache is None:
                children_distribution = get_clade_count(
                    node, clades=clades, alternate_names=alternate_names
                )
            else:
            ...
            if (
                density >= clade_details[MIN_DENSITY]
                and density <= clade_details[MAX_DENSITY]
                and coverage >= clade_details[MIN_COV]
                and coverage <= clade_details[MAX_COV]
            ):
    ...
    qualifying_list = get_qualifying_nodes(
        tree,
        clade=clade,
        clade_details=clade_details,
    ...
    sorted_nodes = list(
        reversed(
            sorted(qualifying_list.keys(), key=lambda x: qualifying_list.get(x)[SCORE])
        )
    )
    try:
        print(
        ...
        raise Exception(
            f"Could not find any node to color for clade {clade} that fulfills criteria: {clade_details}. Try lowering the minimum Coverage and/or density value for clade {clade}"
        )
...
def color_clades(
    tree: TreeNode,
    clades: List[str],
    clade_details: Dict[str, List],
    clade_counts: Dict[str, int],
    node_cache: Dict[TreeNode, Dict[str, int]],
):
    """
    ...
    """
    for clade in clades:
        max_ancestor = get_max_ancestor(
            tree,
            clade=clade,
            clade_details=clade_details[clade],
            clade_total=clade_counts[clade],
            node_cache=node_cache,
        )
        max_ancestor.img_style = node_bg_style(
            max_ancestor.img_style, clade_details[clade][CLADE_COL]
        )
